...
export default class ChromaKeyOperation extends Operation {
    ...
    draw(ctx, relativeFrame) {
        ...
        const foregroundImgData = this.foregroundCtx.getImageData(0, 0, this.foregroundCtx.canvas.width, this.foregroundCtx.canvas.height);
        const backgroundImgData = this.backgroundCtx.getImageData(0, 0, this.backgroundCtx.canvas.width, this.backgroundCtx.canvas.height);
        const [tr, tg, tb] = this.color; 
        for (let i = 0; i < foregroundImgData.width * foregroundImgData.height * 4; i += 4) {
            ...
            if (Math.abs(tr - r) + Math.abs(tg - g) + Math.abs(tb - b) > this.tolerance) {
